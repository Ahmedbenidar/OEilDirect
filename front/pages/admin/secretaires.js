...
export default function AdminSecretaires() {
    ...
    const handleAdd = async (e) => {
        ...
        try {
            await fetchApi('/admin/utilisateurs', { method:'POST', body: JSON.stringify({...form, role:'SECRETAIRE'}) });
            closeModal(); load();
    ...
    const inp = (name, label, opts={}) => (
        <div className="flex flex-col gap-1">
            ...
            <input {...opts} value={form[name]} onChange={e => setForm({...form,[name]:e.target.value})}
                className="h-10 px-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none text-slate-900 dark:text-slate-100 text-sm transition-all"/>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 font-sans">
            <Head>
                <title>Secretaires | Admin OeilDirect</title>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                    <div>
                        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">Gestion des <span className="text-violet-600">Secretaires</span></h1>
                        <p className="text-slate-500 text-sm mt-1">{users.length} secretaire{users.length!==1?'s':''} inscrit{users.length!==1?'s':''}</p>
                    </div>
                    <button onClick={openAdd} className="flex items-center gap-2 bg-violet-600 hover:bg-violet-700 text-white font-semibold px-5 py-2.5 rounded-xl transition-colors shadow-md shadow-violet-200 dark:shadow-none">
                        <span className="material-symbols-outlined text-base">person_add</span>Ajouter une secretaire
                    </button>
                </div>
                ...
                <div className="relative mb-6">
                    ...
                    <input type="text" placeholder="Rechercher par nom, CIN ou email..." value={search} onChange={e => setSearch(e.target.value)}
                        className="w-full pl-11 pr-4 h-11 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none transition-all text-slate-900 dark:text-slate-100 placeholder:text-slate-400"/>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                    {loading ? (
                        ...
                        <div className="p-16 text-center">
                            ...
                            <p className="mt-3 text-slate-500">Aucune secretaire trouvee.</p>
                        </div>
                    ) : (
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                                    <th className="text-left px-5 py-3.5 font-semibold text-slate-600 dark:text-slate-400">Secretaire</th>
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-9 h-9 rounded-full bg-violet-100 dark:bg-violet-900/40 flex items-center justify-center flex-shrink-0">
                                                    <span className="text-violet-600 dark:text-violet-400 font-bold text-sm">{(u.prenom||u.nom||'?').charAt(0).toUpperCase()}</span>
                                                </div>
                                                <div>
                            ...
                            <form onSubmit={modal==='add'?handleAdd:handleEdit} className="p-6 flex flex-col gap-4">
                                <div className="flex items-center justify-between mb-1">
                                    <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">
                                        {modal==='add'?'Ajouter une secretaire':'Modifier la secretaire'}
                                    </h2>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {inp('prenom', 'Prenom *', {required:true, placeholder:'Fatima'})}
                                    {inp('nom', 'Nom *', {required:true, placeholder:'Benali'})}
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    {inp('age', 'Age', {type:'number', min:0, max:80, placeholder:'28'})}
                                    {inp('cin', 'CIN', {placeholder:'123456789'})}
                                </div>
                                {inp('telephone', 'Numero de telephone', {placeholder:'+213 6XX XXX XXX'})}
                                {inp('email', 'Email *', {required:true, type:'email', placeholder:'secretaire@exemple.com'})}
                                <div className="flex flex-col gap-1">
                                    ...
                                    <input type="password" required={modal==='add'} value={form.motDePasse}
                                        ...
                                        className="h-10 px-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none text-slate-900 dark:text-slate-100 text-sm"/>
                                </div>
                                <div className="flex gap-3 mt-2">
                                    ...
                                    <button type="submit" disabled={submitting} className="flex-1 py-2.5 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-semibold text-sm disabled:opacity-60">
                        ...
                        {modal==='delete' && (
                            <div className="p-6 flex flex-col gap-5">
                                <div className="flex items-center justify-between">
                                    <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Supprimer la secretaire</h2>
